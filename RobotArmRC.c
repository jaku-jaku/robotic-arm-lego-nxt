...
//-----------------------------------------//
//-----Some Trivial Functions for main-----//
//-----------------------------------------//
void message(int line, string sString){
	int size=16;
	int cnt=strLen(sString)/size;
	for(int i=0;i<cnt;i++){
		nxtDisplayString(line+i, sString);
		StringDelete(sString, size*i, size);
	}
};
//----------------------//
//-----Main Program-----//
...
task main()
{
  ...
  nxtDisplayCenteredTextLine(0, "Ready to Calibrate");
  message(2, "Press Any Button to begin");
  waitBtn();//Wait for starting the program
  nxtDisplayCenteredTextLine(0, "Caution");
  message(2,"Auto Calibration Starts in 2 seconds, Step back!");
  PlaySound(soundBlip);
  ...
  ClearSounds();
	...
	message(2, "Press Any Button to begin the task");
	waitBtn();
	//Declare Variables
